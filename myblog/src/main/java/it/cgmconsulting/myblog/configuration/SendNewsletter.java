...
public class SendNewsletter {
    ...
    @Scheduled(fixedRate = 5000) //in millisecondi, intervalli fissi
    //@Scheduled(cron = "* * * * * *") //weekly in modo strano - vedi documentazione
    //@Scheduled(cron ="@weekly")//weekly in modo umano
    public void sendNewsletterSched() {
        log.info("o-o-o-o-o-o-o-o-o-o");

        // Recupero i dati del consenso degli utenti per quelli che hanno una frequency != NEVER ed invio una mail con i post pubblicati
        ...
        for (Post p : weeklyPosts) {
            weeklyMailContent.append("<br> <a href='http://localhost:8090/api/v0/posts/")
                    .append(p.getId()).append("'>").append(p.getTitle()).append("</a>");
        }
    ...
    private boolean validWeeklyConsent(Consent c) {
        return (c.getLastSent() == null || LocalDate.now().minusWeeks(1).isAfter(c.getLastSent().minusDays(1)))
                && c.getConsentId().getUserId().isEnabled();
    }

    private boolean validMonthlyConsent(Consent c) {
        return (c.getLastSent() == null || LocalDate.now().minusMonths(1).isAfter(c.getLastSent().minusDays(1)))
                && c.getConsentId().getUserId().isEnabled();
    }
