...
public class CommentService {
    ...
    /*
    public CommentResponse updateComment(int commentId, String updatedComment) {

        Comment comment = commentRepository.findById(commentId)
                .orElseThrow(() -> new ResourceNotFoundException("Comment", "id", commentId));

        if (comment.getAuthor() != author) {
            throw new GenericException("You are not authorized to update this comment");
        }
        ...
        return new CommentResponse(
                comment.getId(),
                comment.getComment(),
                String.valueOf(comment.getAuthor())
        );
    }
    */
